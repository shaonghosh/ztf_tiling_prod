...
class ZTFtile:
    ...
    classlocation=os.path.split(__file__)[0]
    try:
        t=Table.read(os.path.join(classlocation,'ZTF_quadrantcenters.dat'),format='ascii.commented_header')
    except IOError:
        raise IOError('Unable to read ZTF quadrant center file %s' % os.path.join(classlocation,'ZTF_quadrantcenters.dat'))
